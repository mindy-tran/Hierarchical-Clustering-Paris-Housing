import pandas as pd
import numpy as np
from agglomerative import AgglomerativeAlgorithm
...
class Experiment01:
    ...
    def run():
        ...
        :return: None
        """
        X_data = Experiment01._load_data()

        # remove outliers
        X_data = Experiment01.remove_outliers(X_data)

        # normalization before computing distances
        X_data = Experiment01.normalize(X_data)

        # Elbow Method to Find the Optimal Number of Clusters
        Visualization.vis_elbow('Agglomerative', X_data)
        # we see that about 5 clusters are good

        # hierarchical clustering model training on the data
        agglo_clustering = AgglomerativeAlgorithm()
        # give each point a cluster number
        y_hc = agglo_clustering.fit_predict(X_data)
        data_labels = agglo_clustering.Agg_hc.labels_
        ...
        # visualize clusters
        Visualization.vis_clusters('Agglomerative', 'squareMeters', 'price', X_data, 'Cluster')

        # boxplot to evaluate the ML technique
        Visualization.boxplot('Agglomerative', 'Cluster', ['squareMeters', 'price'], X_data)
